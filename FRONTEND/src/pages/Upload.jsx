...
import { matieresAPI, ressourcesAPI } from '../utils/api'
...
export default function Upload() {
  ...
  const [form, setForm] = useState({ titres_ressources:'', type_ressources:'cours', filiere:'L1', matiere:'', description:'', file:null })
  const [loading, setLoading] = useState(false)

  // Charger les matières en fonction de la filière sélectionnée
  useEffect(() => {
    const params = form.filiere ? { filiere: form.filiere } : {}
    matieresAPI.list(params).then(r => setMatieres(r.data.data))
  }, [form.filiere])
  ...
  const set = k => e => setForm(f => ({ ...f, [k]: e.target.value }))
   ...
   const handle = async e => {
      ...
      if (!form.filiere) return toast('Veuillez sélectionner une filière', 'error')
      if (matieres.length > 0 && !form.matiere) return toast('Veuillez sélectionner une matière', 'error')
     setLoading(true)
      try {
      ...
      <Card style={{ padding:'32px' }} className="fade-up">
        <form onSubmit={handle} style={{ display:'flex', flexDirection:'column', gap:'18px' }}>
          <Input label="Titre de la ressource *" placeholder="Ex: Cours Bases de Données – L3 GLSI"
           ...
           <Select label="Filière *" value={form.filiere} onChange={set('filiere')}>
             {FILIERES.map(f => <option key={f.value} value={f.value}>{f.label}</option>)}
           </Select>
<Select label="Matière *" value={form.matiere} onChange={set('matiere')} required disabled={matieres.length === 0}>
  <option value="">{matieres.length === 0 ? 'Aucune matière disponible' : 'Sélectionnez une matière'}</option>
